function showCreateForm() {
    document.getElementById('createScooterForm').style.display = 'block';
}
...
function appendListItem(list, item) {
    ...
    deleteButton.addEventListener('click', deleteScooter.bind(null, item.id));
    ...
    detailsLink.addEventListener('click', showScooterDetails.bind(null, item.id, item.detailsId));
...
function showEditForm(id, manufacturer, model, year, detailsId) {
    const editForm = document.getElementById('editScooterForm');
    ...
    button.setAttribute('onclick', `editScooter(${id})`);
}
