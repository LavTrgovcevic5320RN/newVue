function showCreateForm() {
    document.getElementById('createBicycleForm').style.display = 'block';
}
...
function appendListItem(list, item) {
    ...
    deleteButton.addEventListener('click', deleteBicycle.bind(null, item.id));
    ...
    detailsLink.addEventListener('click', showBicycleDetails.bind(null, item.id, item.detailsId));
...
function showEditForm(id, manufacturer, model, year, detailsId) {
    ...
    button.setAttribute('onclick', `editBicycle(${id})`);
}
